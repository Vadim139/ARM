...
void GPIO_PinInit(GPIO_TypeDef* Port, uint16_t Pin, GPIOMode_TypeDef Mode, GPIOOType_TypeDef Type, GPIOPuPd_TypeDef PuPd, GPIOSpeed_TypeDef Speed)
{
  ...
  switch ((uint32_t)Port) {
	case GPIOA_BASE :
		RCC_AHBPeriphClockCmd((uint32_t)RCC_AHBPeriph_GPIOA, ENABLE);
		break;
	case GPIOB_BASE :
		RCC_AHBPeriphClockCmd((uint32_t)RCC_AHBPeriph_GPIOB, ENABLE);
		break;
	case GPIOC_BASE :
		RCC_AHBPeriphClockCmd((uint32_t)RCC_AHBPeriph_GPIOC, ENABLE);
		break;
	case GPIOD_BASE :
		RCC_AHBPeriphClockCmd((uint32_t)RCC_AHBPeriph_GPIOD, ENABLE);
		break;
	case GPIOE_BASE :
		RCC_AHBPeriphClockCmd((uint32_t)RCC_AHBPeriph_GPIOE, ENABLE);
		break;
	case GPIOF_BASE :
		RCC_AHBPeriphClockCmd((uint32_t)RCC_AHBPeriph_GPIOF, ENABLE);
		break;
	default:
		break;
}
...
{
	GPIO_PinInit(Port,Pin,GPIO_Mode_OUT,GPIO_OType_PP, GPIO_PuPd_UP, GPIO_Speed_Level_3);
}

void GPIO_PinIn(GPIO_TypeDef* Port, uint16_t Pin)
{
	GPIO_PinInit(Port, Pin, GPIO_Mode_IN,GPIO_OType_PP, GPIO_PuPd_UP, GPIO_Speed_Level_3);
}
...
void LED_init(void) {
	...
	STM_EVAL_LEDInit(LED7);       // 1s = 72000000  2s = 72000000 * 2
	STM_EVAL_LEDInit(LED8);       // 1 ms = 72000  1 qs = 72
...
// TIM2 - 16 bit prescaler, 32 bit counter
...
//prescaler--;
	...
	RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOA, ENABLE);	//enable the AHB Peripheral Clock to use GPIOB
...
//prescaler--;
	...
	RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOC, ENABLE);	//enable the AHB Peripheral Clock to use GPIOB
	...
	if (channel == 1)
	{
		...
		GPIO_PinAFConfig(GPIOC, GPIO_PinSource6, GPIO_AF_4);//connect the pin to the desired peripherals' Alternate Function (AF) - TIM4 (GPIO_AF_2)
	}
	else if (channel == 2)
	{
...
void TIM_Config(uint32_t TIM, uint16_t prescaler, uint32_t counter) //2, 3, 4, 6, 7 timers
{
	...
	if (TIM == TIM1)
		...
		nvicStructure.NVIC_IRQChannel = TIM8_BRK_IRQn;
