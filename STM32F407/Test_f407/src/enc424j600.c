...
void enc424j600Init(void) {
    ...
    mac_addr[0] = ((u08*) & regValue)[0];
    mac_addr[1] = ((u08*) & regValue)[1];
    regValue = enc424j600ReadReg(MAADR2);
    mac_addr[2] = ((u08*) & regValue)[0];
    mac_addr[3] = ((u08*) & regValue)[1];
    regValue = enc424j600ReadReg(MAADR3);
    mac_addr[4] = ((u08*) & regValue)[0];
    mac_addr[5] = ((u08*) & regValue)[1];
...
u16 read_reg(u16 address)
{
	return enc424j600ReadReg(address);
}

void write_reg(u16 address, u16 data)
{
	enc424j600WriteReg(address, data);
}
...
//    while (!(SPSR & (1 << SPIF)));
	...
	TM_SPI_ReadMulti(SPIX, data, 0x00, dataLen);
//    while (dataLen--) {
//        // wait for answer
//        SPDR = 0x00;
...
//    while (!(SPSR & (1 << SPIF)));
	...
	TM_SPI_WriteMulti(SPIX, data, dataLen);
//    while (dataLen--) {
...
//	CS_H();
    ...
    // read answer
//	TM_SPI_ReadMulti(SPIX, dummy, 0x00, 1);
	CS_H();
}
...
//	TM_SPI_Send(SPIX, data);
	SPI_I2S_SendData(SPIX,op);
	SPI_I2S_SendData(SPIX,data);
	returnValue = SPI_I2S_ReceiveData(SPIX);

//	TM_SPI_ReadMulti(SPIX, returnValue, 0x00, 1);
    // release CS
	CS_H();
...
//	TM_SPI_Send(SPIX, op);
	...
	for (int x = 0; x < 2; x++) {
		SPI_I2S_SendData(SPIX,((u08*) & data)[x]);
		((u08*) & returnValue)[x] = SPI_I2S_ReceiveData(SPIX);
	// start sending data to SPI
//	TM_SPI_Send(SPIX, ((u08*) & data)[x]);
    // read answer
//	TM_SPI_ReadMulti(SPIX, ((u08*) & returnValue)[x], 0x00, 1);
}
