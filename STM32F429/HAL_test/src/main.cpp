...
void LCD_puts(int x, int y, uint8_t data)
{
	char buf[10];
	data = 200;
...
//	RCC_HSEConfig(RCC_HSE_ON);
	...
	__HAL_RCC_MCO1_CONFIG(RCC_MCO1SOURCE_HSE,RCC_MCODIV_1);//hse
...
//	Cam_Rst.Set_low();
	_delay_ms(100);
//	SCCB_GPIO_Config();
	TM_I2C_Init(SCCB_Camera,TM_I2C_PinsPack_1,TM_I2C_CLOCK_STANDARD);
...
//	GPIO_InitTypeDef   GPIO_InitStructure;
	...
	/* D2..4,6..7(PE0/1/4/5/6) */
	GPIO_InitStructure.Pin = GPIO_Pin_0|GPIO_Pin_1|GPIO_Pin_4|GPIO_Pin_5|GPIO_Pin_6;
...
//	void DCMI_Config(){
	  ...
	  DCMI_HandleTypeDef DCMI_InitDef;

	  /*** Configures the DCMI to receive data from MT9D111 ***/

	  /* DCMI configuration */
	  DCMI_InitStructure.CaptureMode = DCMI_CaptureMode_Continuous;
