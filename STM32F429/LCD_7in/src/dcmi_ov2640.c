...
//  GPIO_PinAFConfig(GPIOC, GPIO_PinSource5, GPIO_AF_DCMI);
  ...
  /* DCMI GPIO configuration   | GPIO_Pin_7 | GPIO_Pin_11 |
...
void OV2640_Init(ImageFormat_TypeDef ImageFormat)
{
  ...
  DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t) (&(LCD_RAM));//0x60020000;//(uint32_t)bufor+2;//0x60020000;//(uint32_t)frame_buffer;//(uint32_t)bufor;//LCD->Data;//FSMC_LCD_ADDRESS;   // Rejestr 16-bitowy
...
void OV2640_VGAConfig(void)
{
  ...
  /* Initialize OV2640 */
  for(i=0; i<(sizeof(OV2640_VGA)/2); i++)
  {
    OV2640_WriteReg(OV2640_VGA[i][0], OV2640_VGA[i][1]);
    _delay_ms(1);
  }
}
